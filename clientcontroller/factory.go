...
func NewConsumerController(config *fpcfg.Config, logger *zap.Logger, fg *finalitygadget.FinalityGadgetCustom) (api.ConsumerController, error) {
	var (
	...
	case OPStackL2ConsumerChainType:
		ccc, err = opstackl2.NewOPStackL2ConsumerController(config.OPStackL2Config, fg, logger)
		if err != nil {
	...
	case GenericChainType:
		ccc, err = generic.NewGenericConsumerController(config.ConsumerGenericConfig, fg, logger)
		if err != nil {
